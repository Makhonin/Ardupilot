...
static void read_radio()
{
    ...
    if (hal.rcin->valid_channels() > 0) {
        ...
        int32_t roll_angle2 = (wrap_180_cd(get_rate_roll(roll_rate_target_bf)))/5;//get_rate_roll(roll_rate_target_bf);
		if ( periods[7] > CONV_THROTTLE )
		{ //qua
		hal.rcout->enable_ch(6);
		hal.rcout->write(6, 1000);
		hal.rcout->enable_ch(7);
		hal.rcout->write(7, 2000);
		...
		hal.rcout->enable_ch(6);
                hal.rcout->write(6, (1500-g.p_conversion+roll_angle2)*10/4+1000);
                hal.rcout->enable_ch(7);
                hal.rcout->write(7, 2000-(1500-g.p_conversion-roll_angle2)*10/4);
                hal.rcout->set_freq(0xC0, 50);
                /*hal.rcout->enable_ch(6);
